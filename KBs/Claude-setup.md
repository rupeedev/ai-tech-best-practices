...
- Use `Shift + Tab` twice to enter plan mode before coding
- Plan your architecture before asking Claude to build anything
- Have detailed back-and-forth discussions to settle on a solution
- Be specific: "Build email/password auth with Redis sessions" vs "build me an auth system"

### 2. CLAUDE.md File

- A markdown file Claude reads at the start of every session
- **Keep it short** - Claude can follow ~150-200 instructions; system prompt uses ~50
- **Make it project-specific** - don't explain basics, tell it the weird/unique stuff
- **Tell it why, not just what** - context helps Claude make better judgment calls
- Update constantly using `#` key to add instructions automatically

### 3. Manage Context Windows

- Quality degrades at **20-40%** context usage, not 100%
- **Scope conversations** - one conversation per feature/task
- Use **external memory** (`SCRATCHPAD.md`, `plan.md`) for complex work
- **Copy-paste reset trick**: copy important info → `/compact` → `/clear` → paste back
- Use `/clear` when conversations go off rails

### 4. Prompting Best Practices

- Be specific about what you want
- Tell Claude what **NOT** to do (avoid over-engineering)
- Give context about **why** (constraints, speed requirements, prototype vs production)
- Bad output = bad input

### 5. Model Selection

- **Sonnet**: Faster, cheaper - good for execution tasks with clear paths
- **Opus**: Slower, expensive - better for complex reasoning and planning
- **Workflow**: Use Opus for planning → switch to Sonnet (`Shift+Tab`) for implementation

### 6. Tools & Configuration

- **MCP servers**: Connect to Slack, GitHub, databases, APIs
- **Hooks**: Run code automatically (Prettier, type checking) after Claude edits
- **Custom slash commands**: Create `.claude/commands` folder with markdown files

### 7. When Claude Gets Stuck

- `/clear` the conversation for fresh start
- Break complex tasks into smaller pieces
- Show examples instead of explaining repeatedly
- Reframe the problem differently

### 8. Build Systems (Headless Mode)

- Use `-p` flag for headless mode (non-interactive)
- Automate: PR reviews, support tickets, documentation updates
- Create a feedback flywheel: Claude mistakes → review logs → improve CLAUDE.md → better results
...
Modern day technology is absurdly capable. If you want more tips on how to get the most out of AI for yourself or your business, subscribe to my free weekly AI newsletter: 

[https://varickagents.com/newsletter](https://varickagents.com/newsletter)

 File: plan.md
  Purpose: Implementation blueprint - Detailed technical plan for a specific feature/task
  When to Use: Before coding - architecture, phases, decisions, acceptance criteria
  ────────────────────────────────────────
  File: SCRATCHPAD.md
  Purpose: Working notes - Temporary info, observations, quick todos
  When to Use: During coding - notes, findings, questions, progress tracking
  Example:

  plan.md (structured, one feature):
...
3. Implement login modal

## Technical Decisions

  | Decision | Choice | Rationale |
  | Auth provider | Clerk | Already in stack |

  SCRATCHPAD.md (informal, ongoing):

## Current Focus

  Working on IKA-77 file upload

## Notes

- Found bug in useUpload hook line 45
- Need to ask about max file size limit

## TODO

- [ ] Fix the upload progress bar
- [ ] Test with large files

  TL;DR: plan.md = think before you build, SCRATCHPAD.md = notes while you work.
